...
export class TwinController {
  ...
  @ApiOperation({
    ...
    description: 'Returns the current status of the GCS twin (running, healthy, port, and URL)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    summary: 'Start twin',
    description: 'Starts the GCS twin Docker container and seeds initial data. Requires Docker to be running.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        message: { type: 'string', example: 'Failed to start GCS twin: Docker is not running' },
      },
    },
  })
  async start() {
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        success: { type: 'boolean', example: false },
        message: { type: 'string', example: 'Failed to stop GCS twin: Container not found' },
      },
    },
  })
  async stop() {
