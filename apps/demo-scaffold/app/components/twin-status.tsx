import { useTwinControllerGetStatusQuery, useTwinControllerStartMutation, useTwinControllerStopMutation } from '@open-kingdom/shared-frontend-data-access-api-client';
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Port {port} • {running ? (healthy ? 'Healthy' : 'Unhealthy') : 'Stopped'}
          </p>
        </div>
        <div className="flex items-center space-x-2">
          <div
            className={`h-3 w-3 rounded-full ${running ? (healthy ? 'bg-green-500' : 'bg-yellow-500') : 'bg-gray-400'}`}
            title={running ? (healthy ? 'Running and healthy' : 'Running but unhealthy') : 'Stopped'}
          ></div>
